      ...
      AND ba.attraction_id = "${action.attractionId}"`
		...
		if (result[0].length === 0)
			await db.query(
				...
				VALUES("${action.userId}", "${action.attractionId}", "${action.price}")`
			)
		res.end()
	} catch (err) {
		console.log(err)
...
router.post('/register', async function(req, res) {
	try {
		...
		let newUser = await db.query(
			`INSERT INTO user 
			VALUES(null,'${user.email}','${encryptedPassword}')`
		)
		await db.query(
			`INSERT INTO 
				weddingdetails 
		...
		res.send({ newUser, message: 'Congrats! you have successfully registered.' })
	} catch (err) {
		res.status(400).json({ message: err.message })
	}
})
...
router.put('/invitee/addtotable', async (req, res) => {
	try {
		...
		if (inviteeOldTable)
			await db.query(
				`UPDATE tables 
				SET seated = "${inviteeOldTable.seated - addSeatsNum}" 
				WHERE id="${inviteeOldTable.id}"`
			)
		await db.query(
			`UPDATE 
				invitee 
